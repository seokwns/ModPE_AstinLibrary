...
const Utils = {
	...
	uiThread : function(func) {
		pe.CONTEXT.runOnUiThread(new Runnable({ run : function() {
			try { 
				func(); 
			} catch(err) { 
				Utils.Debug(err); 
	...
	Download : (url, path, name) => {
		Utils.Thread(() => {
			...
			var _url = new java.net.URL(url), urlConnect = _url.openConnection();
			urlConnect.connect();
...
pe.seize.widget.PopupWindow = function() {
	...
	this.menuBtn = new pe.seize.widget.Button()
				...
				.setParams(20 * Utils.DP, 20 * Utils.DP)
	...
	this.titleLayout.addView(this.title, WIDTH - 70 * Utils.DP, 45 * Utils.DP);
	
	this.close = new pe.seize.widget.CircleButton()
				.setText("")
				.setRadius(10 * Utils.DP)
	...
	this.mainLayout.addView(this.titleLayout, -1, 45 * Utils.DP);
};
...
pe.seize.widget.PopupWindow.PopupStyle = {
	GRID_LEFT : 0, GRID_TOP : 1, GRID_RIGHT : 2, GRID_BOTTOM : 3, OVERLAY : 4
};
...
pe.seize.widget.ProgressBar = function(style, width, height) {
	...
	switch(style) {
		case 0:
			...
			break;
		case 1: case 2:
			this.bar = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888);
			break;
			
		default : 
			Utils.Toast("그런 프로그래스바 스타일은 존재하지 않습니다!");
			break;
	}
